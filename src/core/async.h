...
el_task_t *el_setTimeout(el_task_t *handler, void callback(), el_time_t ms, fun_params_t *params);
el_ret_t el_clearTimeout(el_task_t *taskInstance);
el_task_t *el_setInterval(el_task_t *handler, void callback(), el_time_t ms, fun_params_t *params, task_immediate_t immediate);
